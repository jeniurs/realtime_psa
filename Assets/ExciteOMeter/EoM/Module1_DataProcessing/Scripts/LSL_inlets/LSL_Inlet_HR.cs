...
namespace ExciteOMeter
{
    public class LSL_Inlet_HR : InletShortSamples
    {
        ...
        void Awake()
        {
            folder = Path.Combine(Application.dataPath, "HR_CSV");
        ...
        void Start()
        {
            ...
            Debug.Log($"[LSL_HR] Started looking for stream: Name='{StreamName}', Type='{StreamType}'");
        }
        ...
        // ✅ sampleUtcMs를 받아서 “첫 샘플 기준”으로 파일명/기준시각 통일
        private void EnsureWriter(long sampleUtcMs)
        {
            ...
            string fileName = $"HR_{sid}_{stamp}_{baseUtcMs}.csv";
            string path = Path.Combine(folder, fileName);

            writer = new StreamWriter(path, false, Encoding.UTF8, 64 * 1024);
            // 헤더 (한 번만)
            writer.WriteLine("utc_ms,utc_iso,lsl_ts,hr,session,session_start_utc_ms,session_start_utc_iso");

            openedSessionId = sid;

            Debug.Log("[HR] CSV path = " + path);
        }

        protected override void Process(short[] newSample, double timeStamp)
        {
            float hr = newSample[0];

            // 모달리티 준비 여부는 세션과 무관하게 한 번만 알림
            if (!hasLoggedFirstSample)
            {
                hasLoggedFirstSample = true;
                Debug.Log($"[LSL_HR] Received HR data: {hr} (timestamp: {timeStamp:F6})");
                // EyeAudioServer에 HR 모달리티 수신 알림
                EyeAudioServer.NotifyHRReceived();
            }
